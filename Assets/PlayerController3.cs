...
public class PlayerController3 : MonoBehaviour
{
    ...
    public float Timer;
    public bool jumperon = false;
    public Animator camera;
    public float horz = 0f;
    ...
    void Update()
    {
        ...
        if (!inWater)
        {
            ...
            if (jumperon == true && (!GetComponent<LedgeClimbController>() || GetComponent<LedgeClimbController>() && GetComponent<LedgeClimbController>().canGrabBackOn))
            {
        ...
        //do shit here RRRRRRRRRRRRRRRRRRRRRRRRRRRRREEEEEEEEEEEEEEEEEEEEEEEEEEE
    ...
    void GettingRotationDirection()
    {
        ...
        directionVector = new Vector3(horz, 0, vertz);
        ...
        if (inMidAirFromWallJump)
        {
            ...
            if (jumpedOffWallForWallJump)
            {
                ...
                if (noCollisionTimer >= 5 && !currentlyOnWall)
                {
                    ...
                    {
                        wallJumpDirection += (horz * (wallJumping.overallMovementSpeed / 8) * playerCamera.transform.right);
                        wallJumpDirection += (vertz * (wallJumping.overallMovementSpeed / 8) * playerCamera.transform.forward);
                    }
                    else
                    {
                        if (movement.sideScrolling.lockMovementOnXAxis)
                        {
                            ...
                            if (!movement.sideScrolling.flipAxisRotation)
                            {
                                ...
                                {
                                    wallJumpDirection += (-horz * (wallJumping.overallMovementSpeed / 8) * transform.forward);
                                }
                                else
                                {
                                    wallJumpDirection += (horz * (wallJumping.overallMovementSpeed / 8) * transform.forward);
        ...
        if (currentlyClimbingWall && !pullingUp)
        {
            ...
            if (directionVector.magnitude != 0)
            {
                ...
                if (climbHorizontally2 && climbVertically2 && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis))
                {
                    ...
                    if (stopAtSides2 && (reachedRightPoint && horz > 0 || reachedLeftPoint && horz < 0)
                    && (!downCanBePressed || reachedTopPoint && vertz > 0 || reachedBottomPoint && vertz < 0))
                    {
                        climbDirection = Vector3.zero;
                    }
                    //if we have reached the left or right point, do not allow horizontal movement in that direction
                    else if (downCanBePressed && stopAtSides2 && (reachedRightPoint && horz > 0 || reachedLeftPoint && horz < 0)
                    && ((!reachedTopPoint || vertz <= 0) && (!reachedBottomPoint || vertz >= 0)))
                    {
                        climbDirection = (vertz * transform.up) * climbingMovement;
                    }
                    //if down cannot be pressed, or we have reached the top or bottom point, do not allow vertical movement in that direction
                    else if (!downCanBePressed || reachedTopPoint && vertz > 0 || reachedBottomPoint && vertz < 0)
                    {
                        climbDirection = (horz * transform.right) * climbingMovement;
                    }
                    ...
                    {
                        climbDirection = (horz * transform.right + vertz * transform.up) * climbingMovement;
                    }
                ...
                else if (climbVertically2)
                {
                    ...
                    if (!downCanBePressed || reachedTopPoint && vertz > 0 || reachedBottomPoint && vertz < 0)
                    {
                        climbDirection = Vector3.zero;
                    }
    ...
    void JumpOffOfClimb()
    {
        ...
        if ((currentlyClimbingWall || turnBack || back2) && Input.GetButtonDown("A/Jump"))
        {
            moveDirection = Vector3.zero;
            swimDirection = Vector3.zero;
            swimmingMovementSpeed = 0;
            ...
            if (!pullingUp)
            {
                ...
                if (currentlyClimbingWall)
                {
                    if (climbHorizontally2 && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis))
                    {
                        ...
                        if (!stopAtSides2 || (!reachedRightPoint || horz < 0) && (!reachedLeftPoint || horz > 0))
                        {
                            animator.SetFloat("climbState", Mathf.Abs(horz) + 1);
                        }
                        else
                        {
                ...
                //horizontal movement
                if (horz > 0 && currentlyClimbingWall && climbMovementSpeed2 > 0 && (!reachedRightPoint || !stopAtSides2))
                {
                ...
                else if (horz < 0 && currentlyClimbingWall && climbMovementSpeed2 > 0 && (!reachedLeftPoint || !stopAtSides2))
                {
                ...
                //vertical movement
                if (vertz > 0 && currentlyClimbingWall && climbMovementSpeed2 > 0 && !reachedTopPoint)
                {
                ...
                else if (vertz < 0 && currentlyClimbingWall && climbMovementSpeed2 > 0 && downCanBePressed && !reachedBottomPoint)
                {
                ...
                if (currentlyClimbingWall && climbMovementSpeed2 > 0)
                {
                    ...
                    if (vertz != 0 && climbVertically2 && climbedUpAlready)
                    {
                        if ((!reachedTopPoint || vertz < 0) && (!reachedBottomPoint || vertz > 0))
                        {
                    ...
                    if (horz != 0 && climbHorizontally2 && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis))
                    {
    ...
    void RotatePlayer()
    {
        ...
        if ((directionVector.magnitude > 0 || (movement.sideScrolling.lockMovementOnXAxis || movement.sideScrolling.lockMovementOnZAxis)) && !currentlyClimbingWall && !turnBack && !back2)
        {
            ...
            float myAngle = Mathf.Atan2(horz, vertz) * Mathf.Rad2Deg;
            ...
            if (!currentlyOnWall)
            {
                ...
                else if ((!movement.firstPerson.walkBackwardsWhenDownKeyIsPressed || vertz >= 0 || horz != 0 || !inFirstPersonMode) && playerCamera.transform.parent != transform)
                {
    ...
    void GettingMovementDirection()
    {
        ...
        if (grounded.currentlyGrounded && (noCollisionTimer < 5 || Physics.Raycast(pos, Vector3.down, maxGroundedDistance2, noWaterCollisionLayers)))
        {
            ...
            if (!jumpPerformed)
            {
                moveDirection = new Vector3(horz, 0, vertz);
            }
            else
            {
                moveDirection = new Vector3(horz, moveDirection.y, vertz);
            }
            ...
            if (directionVector.magnitude != 0)
            {
                ...
                if ((!movement.firstPerson.walkBackwardsWhenDownKeyIsPressed || vertz >= 0 || horz != 0 || !inFirstPersonMode) && playerCamera.transform.parent != transform)
                {
                    //if the player rotates by pressing the arrow keys/moving the joystick
                    if (!movement.firstPerson.onlyRotateWithCamera || !inFirstPersonMode)
                    {
    ...
    void Swim2()
    {
        ...
        if (hPositive)
        {
            hAxis = (Mathf.Abs(horz) - Mathf.Abs(vertz) + 1) / 2;
            if (hAxis != 0)
            {
                hAxis2 = hAxis * 2 - 1;
            }
        ...
        if (vPositive && hPositive && (horz > 0 && vertz > 0 || vertz > 0 && horz > 0)
        || !vPositive && !hPositive && (horz < 0 && vertz < 0 || vertz < 0 && horz < 0)
        || vPositive && !hPositive && (horz > 0 && vertz < 0 || vertz > 0 && horz < 0)
        || !vPositive && hPositive && (horz < 0 && vertz > 0 || vertz < 0 && horz > 0))
        {
            hAxis2 = hAxis;
        ...
        if (vPositive)
        {
            vAxis = (Mathf.Abs(vertz) - Mathf.Abs(horz) + 1) / 2;
            if (vAxis != 0)
            {
                vAxis2 = vAxis * 2 - 1;
            }
    ...
    void Swim3()
    {
        ...
        || (playerCamera.GetComponent<CameraController>() && !playerCamera.GetComponent<CameraController>().goneThroughFirstPersonUpdate))
        {
            ...
            {
                xDeg += horz * swimming.swimRotationSpeed * 0.3f;
            }
            ...
            {
                yDeg += vertz * swimming.swimRotationSpeed * 0.2f;
            }
    ...
    void WallClimbingRotation()
    {
        ...
        if (currentlyClimbingWall && !pullingUp)
        {
            ...
            if ((transform.rotation == lastRot3 || axisChanged) && (climbingMovement > 0 || horz != 0) || hasNotMovedOnWallYet)
            {

                //to the right of player
                if ((horz > 0 || transform.rotation != lastRot2) && (wallIsClimbable))
                {
                ...
                else if ((horz < 0 || transform.rotation != lastRot2) && (wallIsClimbable))
                {
                    if (Physics.Linecast(transform.position + climbingSurfaceDetectorsUpAmount2 + (transform.up * (climbingSurfaceDetectorsHeight2 + 1)) * 0.5625f, transform.position + climbingSurfaceDetectorsUpAmount2 - transform.right / 3 + (transform.up * (climbingSurfaceDetectorsHeight2 + 1)) * 0.5625f, out hit, noWaterCollisionLayers) && hit.transform != null && hit.transform.tag == climbableTag2)
                    {
                        if (horz < 0)
                ...
                if (climbingMovement > 0 || horz != 0 || vertz != 0)
                {
            ...
            if ((horz > 0 && horizontalAxis <= 0 || horz < 0 && horizontalAxis >= 0))
            {
                axisChanged = true;
            }
            else
