...
public class CameraController : MonoBehaviour
{
    ...
    public class LockingOn
    {
        ...
        public string lockOnInputButton = "R/CameraReset"; //the button or axis (found in "Edit > Project Settings > Input") pressed to lock on
...
if (CursorInvisible == true) { 
    ...
    void Update()
    {
        if (Input.GetButtonDown(lockingOn.lockOnInputButton) && lockingOn.allowLockingOn) { cameramove.PlayOneShot(move); }
      ...
      //  }
        ...
        if (barHolder == null)
        {
            Input.GetButtonDown("R/CameraReset");
        ...
        if (firstPerson.crouching.allowCrouching && (!player.GetComponent<PlayerController>() || !firstPerson.crouching.crouchWithPlayerIfPossible))
        {
            ...
            else if ((Input.GetButtonDown("L/Crouch") || Input.GetKeyDown(KeyCode.Joystick1Button8)) && !Physics.Linecast(transform.position, new Vector3(transform.position.x, newYPos, transform.position.z), out hit, collisionLayers) && finishedCrouching)
            {
                finishedCrouching = false;
                crouching = false;
                canCrouch = false;
    ...
    void LockOnVal3()
    {
       ...
       // 
        ...
        //locking on
        if (lockingOn.allowLockingOn && canLockOn && (Input.GetButtonDown(lockingOn.lockOnInputButton) || Input.GetAxis(lockingOn.lockOnInputButton) != 0))
        {
        ...
        else
        {
            ...
            if (lockedOn && (!Input.GetButtonDown(lockingOn.lockOnInputButton) || Input.GetAxis(lockingOn.lockOnInputButton) == 0 || !canLockOn) || !lockingOn.allowLockingOn)
            {
                //cameramove.Play();
                following = true;
                lockedOn = false;
