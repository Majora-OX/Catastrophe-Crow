...
public class PlayerController : MonoBehaviour {
	...
	void Update () {
        ...
        if (anin.GetFloat("speed") > 3.5 && !footstep.isActiveAndEnabled && !anin.GetBool("onLedge"))
        { 
            footstep.enabled = true;
        }
        else if (anin.GetFloat("speed") <= 3.5 || !anin.GetBool("grounded") || anin.GetBool("onLedge")) { footstep.enabled = false; }
       ...
       // footstep.Play();
		...
		if (!inWater){
			...
			if (Input.GetButtonDown("A/Jump") && (!GetComponent<LedgeClimbController>() || GetComponent<LedgeClimbController>() && GetComponent<LedgeClimbController>().canGrabBackOn)){
			...
			if (movement.crouching.allowCrouching){
				...
				if (!canCrouch && (Input.GetButtonDown("L/Crouch") || Input.GetKeyDown(KeyCode.Joystick1Button8)) && finishedCrouching){
				...
				else if ((Input.GetButtonDown("L/Crouch") || Input.GetKeyDown(KeyCode.Joystick1Button8)) && !Physics.Linecast(new Vector3(transform.position.x, feetPositionNew, transform.position.z), new Vector3(transform.position.x, headPositionNew, transform.position.z), out hit, noWaterCollisionLayers) && finishedCrouching){
					finishedCrouching = false;
					crouchCancelsAttack = true;
					crouching = false;
					canCrouch = false;
		...
		if (!currentlyOnWall && !currentlyClimbingWall){
			...
			if (characterController && characterController.enabled){
				...
				||  noCollisionTimer < 5)){
					...
					if ((grounded.currentlyGrounded && !inMidAirFromJump && !inMidAirFromWallJump || inBetweenSlidableSurfaces)
	...
	void Swim3 () {
		...
		if (canPressSwimButton && Input.GetButton("A/Jump")){
	...
	void Swim5 () {
		...
		if ((atJumpHeight || reachedTopOfWater && transform.position.y + 0.01f >= inWaterPosY && !Input.GetButton("A/Jump")) && Mathf.Abs(inWaterPosY - transform.position.y) >= 0.1f && noCollisionTimer >= 5){
			transform.position = new Vector3 (transform.position.x, inWaterPosY, transform.position.z);
		}
		if (atJumpHeight || reachedTopOfWater && transform.position.y + 0.01f >= inWaterPosY && !Input.GetButton("A/Jump") || stillAtTop){
			swimDirection = new Vector3(swimDirection.x, 0, swimDirection.z);
		}
